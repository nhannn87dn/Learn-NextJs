...
const EditProduct = ({product}: {product: ProductType}) => {
    ...
    const handleSubmit = async (event: FormEvent) => {
      ...
      console.log(`Cập nhật thông tin: Name - ${formState.name}, Price - ${formState.price}`);
      try {
      const response = await fetch(`https://64df2d7471c3335b2582313f.mockapi.io/api/v1/products/${product.id}`, {
        method: 'PUT',
        headers: {
    ...
    return <>
    <div>Edit product: {product?.name} - {product?.price}</div>
    <form onSubmit={handleSubmit}>
        ...
        <button  className='py-2 px-4 rounded bg-indigo-500 text-white' type="submit">Cập nhật</button>
      </form>
    </>
}
